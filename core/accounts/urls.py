...
from core.accounts import views as accounts_views
from core.accounts import settings as accounts_settings

urlpatterns = patterns('',
    # Signup, signin and signout
    url(r'^signup/$', accounts_views.signup, name='accounts_signup'),
    url(r'^signin/$', accounts_views.signin, name='accounts_signin'),
    ...
    url(r'^password/reset/confirm/complete/$', auth_views.password_reset_complete, {
                            'template_name': 'accounts/password_reset_complete.html'}),

    # Signup
    url(r'^(?P<username>[\.\w]+)/signup/complete/$', accounts_views.direct_to_user_template, {
    ...
    # Activate
    url(r'^(?P<username>[\.\w]+)/activate/(?P<activation_key>\w+)/$', accounts_views.activate,
                                    name='accounts_activate'),

    # Change email and confirm it
    url(r'^(?P<username>[\.\w]+)/email/$', accounts_views.email_change,
                                    name='accounts_email_change'),
    url(r'^(?P<username>[\.\w]+)/email/complete/$', accounts_views.direct_to_user_template, {
                            'template_name': 'accounts/email_change_complete.html'},
                                    name='accounts_email_change_complete'),
    url(r'^(?P<username>[\.\w]+)/confirm-email/complete/$', accounts_views.direct_to_user_template, {
                            'template_name': 'accounts/email_confirm_complete.html'},
                                    name='accounts_email_confirm_complete'),
    url(r'^(?P<username>[\.\w]+)/confirm-email/(?P<confirmation_key>\w+)/$', accounts_views.email_confirm,
                                    name='accounts_email_confirm'),

    # Disabled account
    url(r'^(?P<username>[\.\w]+)/disabled/$', accounts_views.direct_to_user_template, {
                            'template_name': 'accounts/disabled.html'},
                                    name='accounts_disabled'),

    # Change password
    url(r'^(?P<username>[\.\w]+)/password/$', accounts_views.password_change,
                                    name='accounts_password_change'),
    url(r'^(?P<username>[\.\w]+)/password/complete/$', accounts_views.direct_to_user_template, {
                            'template_name': 'accounts/password_complete.html'},
                                    name='accounts_password_change_complete'),

    # Edit account
    url(r'^(?P<username>[\.\w]+)/edit/account$', accounts_views.account_edit,
                                    name='accounts_account_edit'),

    # Edit profile
    url(r'^(?P<username>[\.\w]+)/edit/profile/$', accounts_views.profile_edit,
                                    name='accounts_profile_edit'),

    # View profiles
    url(r'^(?P<username>(?!signout|signup|signin|password)[\.\w]+)/$', accounts_views.profile_detail,
